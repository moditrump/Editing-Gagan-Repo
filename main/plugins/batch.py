...
@luminant.on(events.NewMessage(incoming=True, pattern='/cancel'))
...
@luminant.on(events.NewMessage(incoming=True, pattern='/logs'))
async def send_log(event):
    ...
    if os.path.exists(temp_log_file):
        ...
        await luminant.send_file(user_id, temp_log_file, caption="Here is the log file of last 2 min.")
    else:
        await event.respond("Log file not found.")


@luminant.on(events.NewMessage(incoming=True, pattern='/batch'))
async def _bulk(event):
    ...
    async with luminant.conversation(event.chat_id) as conv: 
        try:
            ...
            cd = await conv.send_message("**Batch process ongoing...**\n\nProcess completed: ", 
                                    buttons=[[Button.url("Join Channel", url="http://t.me/LuminantBots")]])
    ...
    for i in range(len(ids_data[str(sender)])):
        ...
        try: 
            ...
            await countdown.edit(count_down, 
                                 buttons=[[Button.url("Join Channel", url="https://t.me/LuminantBots")]])
            ...
            if int(fw.value) > 300:
                ...
                try:
                    ...
                    if countdown.text != count_down:
                        await countdown.edit(count_down, buttons=[[Button.url("Join Channel", url="http://t.me/LuminantBots")]])
        except Exception as e:
            #logger.info(e)
            #await client.send_message(sender, f"An error occurred during cloning, batch will continue.\n\n**Error:** {str(e)}")
            if countdown.text != count_down:
                await countdown.edit(count_down, buttons=[[Button.url("Join Channel", url="https://t.me/LuminantBots")]])
        n = i + 1
        if n == len(ids_data[str(sender)]):
            return -2


@luminant.on(events.NewMessage(incoming=True, pattern='/invalid'))
async def _bulk(event):
    user_id = event.sender_id

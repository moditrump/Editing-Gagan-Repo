...
async def start(event):
    ...
    buttons = [
        [Button.url("Join Channel", url="https://t.me/LuminantBots")],
        [Button.url("Contact Me", url="https://t.me/LuminantChatBot")],
    ]
    # Sending photo with caption and buttons
    await luminant.send_file(
        event.chat_id,
        file=START_PIC,
        caption=TEXT,
        buttons=buttons
...
S = "/start"
START_PIC = "https://graph.org/file/818aa312b35052a5c4d74.jpg"
...
@luminant.on(events.NewMessage(func=lambda event: event.photo))
async def save_photo_as_thumbnail(event):
    user_id = event.sender_id
    luminant_client = event.client

    # Download and save the photo as the thumbnail
    temp_path = await luminant_client.download_media(event.media)
...
@luminant.on(events.NewMessage(pattern='/remthumb'))
...
async def youtube_dl_command(_, message):
    ...
    if len(message.command) > 1:
        ...
        try:
            ...
            caption = f"{video_info['title']}\n\n__**Powered by [LUMINANT](https://t.me/LuminantBots)**__"  # Set caption to the title of the video
...
REPO_URL = "https://github.com/LuminantBots"
...
@luminant.on(events.NewMessage(pattern='/help'))
